...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type)
{
  ...
  *rd = BITS(i, 11, 7);
  ...
  case TYPE_J:
    immJ();
    break;
  case TYPE_R: // 新增R型指令处理
    src1R();
    src2R();
    break;
  case TYPE_B: // 新增B型指令处理
    src1R();
    src2R();
    immB();
    break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  // M扩展：乘除法指令（对应测试用例中的mul-longlong.c和div.c）
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul, R, R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(rd) = (int32_t)(((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R, R(rd) = (int32_t)(((int64_t)(int32_t)src1 * (uint64_t)src2) >> 32));
